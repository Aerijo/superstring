...
struct TextBuffer::Layer {
    ...
    vector<TextSlice> chunks_in_range (Range range) {
        ...
        if (result.empty()) result.emplace_back(TextSlice(EMPTY_TEXT));
        return result;
    }

    vector<pair<const char8_t *, uint32_t>> primitive_chunks () {
        vector<pair<const char8_t *, uint32_t>> result;
                ...
                if (last_search_end_position >= chunk_start_position) {
                    ...
                    if (last_match_is_pending) {
                        if (!remaining_chunk.empty() && remaining_chunk.front() == '\n') {
                            chunk_continuation.splice(Point(), Point(), Text { "\r" });
                ...
                switch (match_result.type) {
                    ...
                    case MatchResult::Partial:
                        ...
                        if (chunk_continuation.empty() || match_result.start_offset > 0) {
                            Point partial_match_position = slice_to_search.position_for_offset(match_result.start_offset, minimum_match_row - slice_to_search_start_position.row);
    ...
    vector<SubsequenceMatch> find_words_with_subsequence_in_range (string query, const string &extra_word_characters, Range range) {
        ...
        // subsequence.
        std::unordered_map<string, vector<Point>> substring_matches;
...
void TextBuffer::with_line_for_row (uint32_t row, const std::function<void (const char8_t *, uint32_t)> &callback) {
    string result;
    uint32_t column = 0;
...
void TextBuffer::set_text (string &&new_text) {
    set_text_in_range(Range {{ 0, 0 }, extent() }, move(new_text));
}

void TextBuffer::set_text (const string &new_text) {
    set_text_in_range(Range { Point(0, 0), extent() }, string(new_text));
}
