...
struct Patch::Node {
    ...
    explicit Node (Deserializer &input) :
        ...
        if (input.read<uint32_t>()) {
            old_text = std::make_unique<Text>(input);
            old_text_size_ = 0;
        } else {
            old_text = nullptr;
        ...
        if (input.read<uint32_t>()) {
            new_text = std::make_unique<Text>(input);
        } else {
            new_text = nullptr;
        }
    }
    ...
    void compute_subtree_text_sizes () {
        old_subtree_text_size = old_text_size() + left_subtree_old_text_size() + right_subtree_old_text_size();
        new_subtree_text_size = new_text_size() + left_subtree_new_text_size() + right_subtree_new_text_size();
    }
